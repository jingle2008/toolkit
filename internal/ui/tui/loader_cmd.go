...
func loadBaseModelsCmd(ld loader.Loader, ctx context.Context, kubeCfg string, env models.Environment) tea.Cmd {
	return func() tea.Msg {
		items, err := ld.LoadBaseModels(ctx, kubeCfg, env)
		if err != nil {
			return ErrMsg(fmt.Errorf("failed to load %s: %w", domain.BaseModel, err))
		}
		return baseModelsLoadedMsg{Items: items}
	}
}

func loadGpuPoolsCmd(ld loader.Loader, ctx context.Context, repoPath string, env models.Environment) tea.Cmd {
	return func() tea.Msg {
		items, err := ld.LoadGpuPools(ctx, repoPath, env)
		if err != nil {
			return ErrMsg(fmt.Errorf("failed to load %s: %w", domain.GpuPool, err))
		}
		return gpuPoolsLoadedMsg{Items: items}
	}
}

func loadGpuNodesCmd(ld loader.Loader, ctx context.Context, kubeCfg string, env models.Environment) tea.Cmd {
	return func() tea.Msg {
		items, err := ld.LoadGpuNodes(ctx, kubeCfg, env)
		if err != nil {
			return ErrMsg(fmt.Errorf("failed to load %s: %w", domain.GpuNode, err))
		}
		return gpuNodesLoadedMsg{Items: items}
	}
}

func loadDedicatedAIClustersCmd(ld loader.Loader, ctx context.Context, kubeCfg string, env models.Environment) tea.Cmd {
	return func() tea.Msg {
		...
		return dedicatedAIClustersLoadedMsg{Items: items}
	}
}

func loadTenancyOverrideGroupCmd(ld loader.Loader, ctx context.Context, repoPath string, env models.Environment) tea.Cmd {
	return func() tea.Msg {
		group, err := ld.LoadTenancyOverrideGroup(ctx, repoPath, env)
		if err != nil {
			return ErrMsg(fmt.Errorf("failed to load %s: %w", domain.Tenant, err))
		}
		return tenancyOverridesLoadedMsg{Group: group}
	}
}

func loadLimitRegionalOverridesCmd(ld loader.Loader, ctx context.Context, repoPath string, env models.Environment) tea.Cmd {
	return func() tea.Msg {
		...
		return limitRegionalOverridesLoadedMsg{Items: items}
	}
}

func loadConsolePropertyRegionalOverridesCmd(ld loader.Loader, ctx context.Context, repoPath string, env models.Environment) tea.Cmd {
	return func() tea.Msg {
		...
		return consolePropertyRegionalOverridesLoadedMsg{Items: items}
	}
}

func loadPropertyRegionalOverridesCmd(ld loader.Loader, ctx context.Context, repoPath string, env models.Environment) tea.Cmd {
	return func() tea.Msg {
		...
		return propertyRegionalOverridesLoadedMsg{Items: items}
	}
}
