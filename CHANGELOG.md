...
### Added
- **New `ServingRuntime` category** (alias `sr`), surfacing OME `ClusterServingRuntime` CRs in the TUI, `toolkit get servingruntime`, and the MCP `list_serving_runtimes` tool. Columns: Name, Hardware, Image, Size Range, CPU/Mem, GPU, Auto-Select, Disabled. Three of those are collapsed for the table and kept whole in `-o json`: **Hardware** shows accelerator classes reduced to GPU families with a raw count (`ac: a100-80gb, h100, a10 (19)`), falling back to node-affinity instance types (`it: BM.GPU.H100.8, …`) when no accelerator classes are set — the `ac:`/`it:` marker matters because the two constrain scheduling differently; **Image** drops the registry host and org, which are near-constant across a fleet; **CPU/Mem** pairs the two limits that are read together, leaving GPU its own sortable column. Image, limits, and affinity are read from `spec.engineConfig` in preference to their top-level `spec` duplicates, with a debug log when the two disagree. `autoSelect` is true when *any* `supportedModelFormats` entry sets it. A `disabled` runtime reports as faulty, so it picks up faulty row styling and the faulty-only filter. Namespaced `ServingRuntime` CRs are not included — like `BaseModel`, this is the fleet-level catalog. Filtering matches the raw values, so searching `h100-8` finds a row whose cell only shows `h100`.
