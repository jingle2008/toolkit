...
- `toolkit get importedmodel` (CLI) and MCP `list_imported_models` tool, plus a TUI view (6 columns: Name, Tenant, Namespace, Display Name, Version, Status — operators care about identity + version + status for these; DAC Shape and Flags are surfaced on BaseModel where they're more meaningful). All three surfaces share the same loader; the TUI lazy-loads on first navigation to the category. The TUI's Tenant column shows the resolved `Tenant.Name` (via `Dataset.SetImportedModelMap`) when the OCID suffix matches a tenant in the realm config; the CLI's TENANT column shows the raw OCID (same asymmetry as DAC).
