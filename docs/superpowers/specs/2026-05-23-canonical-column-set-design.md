...
| 6 | `Default bool` on each Column drives the CLI default table; TUI ignores `Default` and shows everything. **Current policy: every column is `Default==true` so CLI defaults mirror the TUI** — `--columns` is a pure narrowing tool. | Separates schema (unified) from presentation (configurable). The all-true policy keeps the field load-bearing as a future per-format curation hook without dropping the type. |
